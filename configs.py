...
aic_configs = {
    ...
    'prepare_dataset_configs': {
        'CAM_ID_LIST': ['c042'],  # 数据集中包含的相机列表
        'DOWN_SAMPLING_RATE': [0.1, 0.99],  # 下采样率 [0]：同相机车辆下采样率，[1]：异相机下采样率
        'TRAIN_VALI_TEST_RATE': [0.5, 0.25, 0.25],  # 训练集比例
        'SAVE_DIR': './dataset',  # 保存目录
        'BATCH_SIZE': [64, 1024],  # 批次大小：[0]：计算 reid 特征  [1]：保存数据集
        'SINGLE_CAM_MTSC': True,  # 单相机场景 MTSC
    },
    'train_configs': {
        ...
        'TRAIN_DIR': './dataset/c041_[0.1, 0.99]/feat_label/train/',  # 训练集目录
        'TEST_DIR': './dataset/c042_[0.1, 0.99]/feat_label/test/',  # 测试集目录
