...
class My_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = torch.from_numpy(self.data[index][-1:]).long() #TODO torch.long()什么意思呢
...
def main():
    ...
    model = Net().to(device)  # 实例化 model #TODO
...
if __name__ == '__main__':
    main() #TODO 1 打印准确率的时候区分 label 和 相机,正确的个数/总个数，对训练集和测试机同样
