...
def get_lane_color_df(lane_color_path):
    ...
    # 计算 [b, g, r]
    lane_color_df['bgr'] = lane_color_df.apply(lambda x: [x.b, x.g, x.r], axis=1) #TODO 为什么使用bgr
...
def main():
    ...
    # 获取场景内全部相机数据集
    if not os.path.isfile(os.path.join(SAVE_DIR, 'img_df.pkl')): #TODO 回看
    ...
    # 确定训练集、验证集、测试集车辆 id
    if not os.path.isfile(os.path.join(SAVE_DIR, 'car_id_list_tvt_dict.pkl')): #TODO 回看
    ...
    # 提取 reid 特征
    if not os.path.isfile(os.path.join(dataset_dir, 'ds_df.pkl')): #TODO 回看
        cfg_path_list = [
